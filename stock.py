...
x = list(data.keys())
y = list(data.values())
source = ColumnDataSource(data=dict(date=x, y=y))
...
plot = figure(plot_height=400, plot_width=400, title="microsoft",
              tools="crosshair,pan,reset,save,wheel_zoom",
                ...
                x_range=['2018-03-01','2018-03-02'], y_range=[-2.5, 5])

plot.line('date', 'y', source=source, line_width=3, line_alpha=0.6)


# Set up widgets
text = TextInput(title="company name", value='microsoft')
textBegin = TextInput(title="begin date", value='2018-03-01')
textEnd = TextInput(title="end date", value='2018-03-02')
